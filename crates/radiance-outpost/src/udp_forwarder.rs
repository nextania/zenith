...
impl UdpForwarder {
    ...
    async fn read_from_socket(socket: Arc<UdpSocket>, outgoing_tx: mpsc::UnboundedSender<(Vec<u8>, String, u16)>) {
        ...
        loop {
            match socket.recv_from(&mut buffer).await {
                Ok((n, addr)) => {
                    ...
                    debug!("UDP read {} bytes from socket (from {}:{})", n, addr.ip(), addr.port());
                }
                Err(e) => {
